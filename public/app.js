...
function renderReport(){
  ...
  root.innerHTML=`<article class="report-sheet">
    ...
    <section class="report-section"><h3>Traffic policy matrix</h3><table><thead><tr><th>Source role</th><th>Destination role</th><th>Action</th><th>Rationale</th></tr></thead><tbody>${policyRows||`<tr><td colspan="4">${roles.length?"A single VLAN role has no cross-zone flows to govern.":"No VLAN roles defined."}</td></tr>`}</tbody></table><p>Policy values describe intended trust boundaries. They are not device configuration.</p></section>
