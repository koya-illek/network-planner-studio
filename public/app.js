...
function sampleState() {
  ...
  return {
    ...
    links:[
      {id:uid(),from:hq,to:branch,type:"vpn",resilience:"dual",routingType:"bgp",transitAllowed:true,defaultRoute:true,advertisedPrefixes:[]},
      {id:uid(),from:hq,to:cloud,type:"vpn",resilience:"single",routingType:"bgp",transitAllowed:true,defaultRoute:false,advertisedPrefixes:[]}
    ]
  };
}
...
function openConnectDialog(fromId,linkId=null){
  ...
  if(link){for(const key of ["from","to","type","resilience","routingType"])form.elements[key].value=link[key]??"";form.elements.transitAllowed.value=String(link.transitAllowed!==false);form.elements.defaultRoute.checked=Boolean(link.defaultRoute);form.elements.advertisedPrefixes.value=(link.advertisedPrefixes||[]).join(", ")}
