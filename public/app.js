...
function renderReport(){
  ...
  const policyRows=roles.flatMap(source=>roles.filter(destination=>destination!==source).map(destination=>{const key=`${source}:${destination}`,value=ENUMS.flowPolicies.includes(state.flowPolicies?.[key])?state.flowPolicies[key]:defaultFlowPolicy(source,destination);return`<tr><td>${escapeHtml(source)}</td><td>${escapeHtml(destination)}</td><td>${escapeHtml(value)}</td><td>${value==="deny"?"Deny intended":value==="allow"?"Allow intended":"Review required before implementation"}</td></tr>`})).join("");
  ...
  root.innerHTML=`<article class="report-sheet">
    <p>Generic IPv4 planning model. Verify cloud-provider address reservations and managed DHCP rules separately. Routes and traffic policies describe design intent; live configuration and reachability are not tested.</p>
...
function animateTrace(linkId){
  ...
  const steps=conflict?["Source VLAN and default gateway","Site edge and route lookup","Trace stopped: destination range overlaps the source site"]:["Source VLAN and default gateway","Site edge and route lookup",`${linkLabel(link)} transport`,"Destination route and VLAN","Modelled path complete"];
